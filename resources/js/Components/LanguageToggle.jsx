export default function LanguageToggle({ bahasa, setBahasa }) {
    ...
    return (
        ...
        >
            <span
                className="absolute top-[3px] left-[3px] w-[31px] h-[24px] rounded-full bg-white shadow-sm transition-transform duration-300 ease-in-out z-10"
                style={{ transform: isEN ? "translateX(0)" : "translateX(31px)" }}
            />
            <span
                className="relative z-20 flex-1 text-center text-[10px] font-bold leading-none transition-colors duration-300"
                style={{ color: isEN ? "#173C33" : "rgba(255,255,255,0.55)" }}
            >
                EN
            </span>
            <span
