...
export default function Games({ boardgames }) {
    ...
    const go = useCallback((overrides = {}) => {
        ...
        router.get(route('games.index'), params, { preserveState: true, replace: true })
    ...
    return (
        <div className="p-6">
            ...
            <Link href={route("games.create")} className="fixed bottom-6 right-6 z-50 btn bg-[#2F6F62] hover:bg-[#255A4F] text-white border-none rounded-full shadow-lg">
                            ...
                            <tr key={game.id} className="hover:bg-slate-50 text-sm">
                                <td className="px-4 py-3">
                                    <div className="flex items-center justify-center gap-2">
                                        <Link href={route("games.show", game.id)} className="btn btn-ghost btn-xs btn-square text-slate-500 hover:text-slate-800 hover:bg-slate-100" title="Detail">
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                        </Link>
                                        <Link href={route("games.edit", game.id)} className="btn btn-ghost btn-xs btn-square text-blue-600 hover:text-blue-800 hover:bg-blue-50" title="Ubah">
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                        </Link>
                                        <Link href={route("games.destroy", game.id)} method="delete" as="button" className="btn btn-ghost btn-xs btn-square text-red-600 hover:text-red-800 hover:bg-red-50" title="Hapus"
